...
async def play(_, message: Message):
    ...
    response = await message.reply_text("🔎 **Sᴇᴀʀᴄʜɪɴɢ...**")
    ...
    if message.chat.id in callsmusic.active_chats:
        ...
        await message.reply_photo(thumb, caption=f"*👉 Yᴏᴜʀ Sᴏɴɢ Qᴜᴇᴜᴇᴅ ᴀᴛ ᴘᴏsɪᴛɪᴏɴ*** `{position}`! \n🏷**ɴᴀᴍᴇ:** [{title[:20]}]({url})\n⏱ **ᴅᴜʀᴀᴛɪᴏɴ:** `{duration}`\n**👑 ʀᴇǫᴜᴇsᴛᴇᴅ ʙʏ: {MENTMEH}**", reply_markup=PLAYMSG_BUTTONS)
    else:
        thumb = THUMB_URL
        await callsmusic.set_stream(message.chat.id, file)
        await response.delete()
        await message.reply_photo(thumb, caption="**☑️ᴘʟᴀʏɪɴɢ ʏᴏᴜʀ sᴏɴɢ...**[{title[:20]}]({url})\n⏱ **ᴅᴜʀᴀᴛɪᴏɴ:** `{duration}`\n**ᴜꜱᴇʀ ʙʏ: {}**".format(message.from_user.mention()), reply_markup=PLAYMSG_BUTTONS)
...
async def nplay(_, message: Message):
    ...
    lel = await message.reply_text("🔎 **Fɪɴᴅɪɴɢ....**")
    ...
    if message.chat.id in callsmusic.active_chats:
        ...
        await callsmusic.set_stream(message.chat.id, file)
        await lel.delete()
        await message.reply_photo(thumb, caption=f"**👉 Pʟᴀʏɪɴɢ ʏᴏᴜʀ sᴏɴɢ**\n🏷**ɴᴀᴍᴇ:** [{title[:20]}]({url})\n⏱ **ᴅᴜʀᴀᴛɪᴏɴ:** `{duration}`\n**👑 ʀᴇǫᴜᴇsᴛᴇᴅ ᴜꜱᴇʀ ʙʏ: {message.from_user.mention}**".format(message.from_user.mention()), reply_markup=PLAYMSG_BUTTONS)
